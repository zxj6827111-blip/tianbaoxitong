...
const REQUIRED_MANUAL_KEYS = [
  { key: 'unit_full_name', expected_source: 'manual_inputs' },
  { key: 'report_contact', expected_source: 'manual_inputs' }
];
...
const rules = [
  ...
  },
  {
    ...
    run: (ctx, config) => {
      ...
      const sumValue = [fiscal, business, other].reduce((sum, item) => sum + (item?.value ?? 0), 0);
      ...
      if (totalValue === null || totalValue === undefined) {
        return [createIssue({
          ...
          message: '预算汇总缺少收入合计',
          tolerance: config.tolerance,
          evidence: buildEvidence({ facts: [total, fiscal, business, other], anchor: 'budget_revenue_total' })
        })];
      }
      ...
      if (!compareWithinTolerance(totalValue, sumValue, config.tolerance)) {
        return [createIssue({
          ...
          tolerance: config.tolerance,
          evidence: buildEvidence({ facts: [total, fiscal, business, other] })
        })];
      }
  ...
  },
  {
    ...
    tolerance: DEFAULT_TOLERANCE,
    run: (ctx, config) => {
      const missingKeys = REQUIRED_MANUAL_KEYS
        .map((item) => item.key)
        .filter((key) => !ctx.manualInputsByKey.has(key));

      if (missingKeys.length > 0) {
        return [createIssue({
          level: config.level,
          rule_id: 'BUDGET.RZ004',
          message: `缺少必填字段：${missingKeys.join(', ')}`,
          tolerance: config.tolerance,
...
const getDraftOrThrow = async (draftId) => {
  const draftResult = await db.query(
    `SELECT id, unit_id, year, upload_id
     FROM report_draft
     WHERE id = $1`,
    [draftId]
  );
...
const evaluateRules = (ctx, configMap) => {
  ...
  for (const rule of rules) {
    ...
    issues.push(...result.map((issue) => ({
      ...issue,
      level: issue.level || effective.level,
...
const runValidation = async (draftId) => {
  ...
  const warn_count = issues.filter((issue) => issue.level === 'WARN').length;
