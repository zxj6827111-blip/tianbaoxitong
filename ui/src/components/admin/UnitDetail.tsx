...
const UnitDetailPanel: React.FC<UnitDetailProps> = ({ unit, year = 2024 }) => {
  ...
  if (!unit) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-slate-400 p-8 text-center animate-fade-in">
         ...
         <p className="text-xs mt-1">查看详细信息与审计记录</p>
      </div>
    );
  }
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {activeTab === 'detail' ? (
          ...
          <div className="h-full animate-fade-in">
            <ArchivePanel departmentId={unit.department_id} year={year - 1} />
          </div>
        )}
      </div>
    </div>
