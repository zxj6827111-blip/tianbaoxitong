...
const formatValue = (value: number | null) => {
  ...
  return `${Number(value).toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} 万元`;
};

const HistoryActualsPanel: React.FC<HistoryActualsPanelProps> = ({ unitId, preferredYear }) => {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const matched = preferred !== null ? nextYears.find((item) => item.year === preferred) : null;
        setSelectedYear((prev) => {
          if (prev && nextYears.some((item) => item.year === prev)) {
            return prev;
          }
          return matched ? matched.year : nextYears[0].year;
        });
      } catch (error) {
        if (!ignore) {
          setYears([]);
  ...
  if (loadingYears) {
    return <div className="p-4 text-sm text-slate-500">正在加载历史归档数据...</div>;
  }
                ...
                <div className="divide-y divide-slate-100">
                  {list.map((field) => (
                    <div key={field.key} className="px-4 py-3 grid grid-cols-[1fr_auto] gap-3 items-center">
                      ...
                      <div className={`text-sm font-medium ${field.value === null ? 'text-slate-400' : 'text-slate-900'}`}>
